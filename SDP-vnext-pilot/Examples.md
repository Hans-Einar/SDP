...
`fixtures/git-history/` creates disposable Git repositories for a valid linear
chain plus disconnected-root, sibling non-ancestor, reversed, duplicate, and
future-candidate negatives. `fixtures/compatibility/` reconstructs the exact
revision-1 and revision-2–4 bytes from Git and rejects revision-5 omission,
revision-6 revision-1 shape, and partial/hybrid forms.
