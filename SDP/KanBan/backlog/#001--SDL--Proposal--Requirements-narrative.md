...
Start requirements work directly in SDL: `01--Actors.design`, followed by `02--UserStories.design` and `03--UseCases.design`. List stakeholders first. Actors may be linked to stakeholders, but this is optional. [KB-SDP-001](../completed/%23001--Proposal--Project-structure.md) owns process placement and studies.
...
`comment` should provide documentable descriptions, distinct from source-only comments. Keyword versus property remains undecided. Compare multiline text (quotes/braces/brackets) and the owner's `#include 01--Actors.design` sketch with the actual grammar before choosing. This card does not introduce an include preprocessor or interpretation of narrative text. [KB-SDL-002](%23002--SDL--Proposal--Links-through.md) owns weak links and visual paths.
...
## Consolidated local consequence — KB-SDP-006

Requirement templates must depend on an adopted SDL narrative profile, never distribute syntax sketches as validated templates. The routing Ref is complete because this primary now
