...
func ClassBundle(ctx context.Context, source string, r Renderer) (*Bundle, error) {
	...
	v := &viewpoint.Views{}
	text := "# Klassedesign — class-core 0.1\n\nEksplisitte klasser/roller/multiplisiteter; ingen strukturrelasjon er omtolket.\n\n" + v.DiagramMarkdown(d, "diagrams/", r != nil) + "| ID | Utsagn | Linje |\n| --- | --- | --- |\n"
	for _, f := range facts {
		text += fmt.Sprintf("| %s | %s | %d |\n", f.S("id"), f.S("text"), f.N("line"))
	}
	b.Put("entry.md", text)
	b.Put("index.md", "# Klassedesign (A4)\n\n[Diagram og kildegrunnlag](entry.md)\n")
	b.Seal()
