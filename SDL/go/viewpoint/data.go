...
func (v *Views) data() {
	...
	for _, d := range v.names("dataset") {
		...
		v.diagram("VP09-data-"+d, "Dataopprinnelse og holder: "+d, join(sources, holders, owners, contracts), d)
	}
	for _, c := range v.names("contract") {
		...
		v.diagram("VP09-contract-"+c, "Kontraktstruktur: "+c, join(variants, fields, eq(r["permits"], "subject", c)), c)
	}
	...
	if len(v.names("dataset")) == 0 {
		v.Gaps = append(v.Gaps, Fact{"viewpoint": "VP09", "model_id": "—", "code": "NO_DATASETS", "message": "Ingen Dataset deklarert."})
	}
	encoded := values(r["encodes"], "object")
	for _, n := range v.names("variant") {
		if !has(encoded, n) {
			v.Gaps = append(v.Gaps, Fact{"viewpoint": "VP10", "model_id": n, "code": "NO_ENCODING", "message": "Ingen fast wirelayout; logisk variant gir ikke packet."})
		}
	}
	if len(v.names("encoding")) == 0 {
		v.Gaps = append(v.Gaps, Fact{"viewpoint": "VP10", "model_id": "—", "code": "NO_ENCODINGS", "message": "Ingen Encoding deklarert."})
	}
}
